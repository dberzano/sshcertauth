...
It is meant to replace [gsissh](http://grid.ncsa.illinois.edu/ssh/), a modified version of both ssh (client) and sshd (server) which directly supports certificate authentication, mostly used for the Grid.
...
Rationale: SSH access for many clients
--------------------------------------
